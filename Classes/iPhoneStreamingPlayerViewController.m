...
@synthesize castURL, castTitle, castImageURL;
...
- (void)viewDidLoad
{
	...
	castImageView.image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:self.castImageURL]]];;
	castTitleLabel.text = self.castTitle;
...
- (NSString*) formatTimeWithInterval:(double)interval {
	...
	NSDate *date3 = [sysCalendar dateFromComponents:conversionInfo];
	
	NSString* time = [format stringFromDate:date3];
...
- (void)updateProgress:(NSTimer *)updatedTimer
{
		...
		if (duration > 0)
		{
			[positionLabel setText:
				[NSString stringWithFormat:@"Time Played: %@  /  %@",
					[self formatTimeWithInterval:progress],
					[self formatTimeWithInterval:duration]]];
	...
	{
		positionLabel.text = @"00:00:00  /  00:00:00";
	}
}
